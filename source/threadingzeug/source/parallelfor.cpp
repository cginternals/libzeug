...
void forEach(std::uint64_t start, std::uint64_t end, std::function<void(std::uint64_t i)> callback, const bool parallelize)
{
    (parallelize ?
        static_cast<void(*)(std::uint64_t, std::uint64_t, std::function<void(std::uint64_t i)>)>(parallelFor) :
        static_cast<void(*)(std::uint64_t, std::uint64_t, std::function<void(std::uint64_t i)>)>(sequentialFor)
    )(start, end, callback);
}
...
void parallelFor(std::uint64_t start, std::uint64_t end, std::function<void(std::uint64_t i)> callback)
{
#ifdef USE_OPENMP
    ...
    {
        callback(static_cast<std::uint64_t>(i));
    }
...
void sequentialFor(std::uint64_t start, std::uint64_t end, std::function<void(std::uint64_t i)> callback)
{
